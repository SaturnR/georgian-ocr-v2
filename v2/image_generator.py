...
def paint_text(text, w, h, rotate=False, ud=False,
               multi_fonts=False, multi_sizes=False,
               save=False, spackle=False, blur=False):
    ...
    if save:
        global img_counter
        img_counter += 1
        surface.write_to_png(create_dir_if_missing('results/gen_imgs/')
                             + 'img_%04d.png' % img_counter)
...
if __name__ == '__main__':
    ...
    for word in args.text:
        img = paint_text(word.decode('utf-8'),
                         args.width, args.height, rotate=True,
                         ud=True, multi_fonts=True, multi_sizes=True,
                         blur=False, save=False)
        mpimg.imsave(os.path.join(
            create_dir_if_missing(args.save_path),
            'img-%s.png' % word.decode('utf-8')),
            img[0], cmap="Greys_r")
